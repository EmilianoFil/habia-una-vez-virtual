...
type TurnoType = typeof TURNOS[number]

const DEFAULT_FORM = { nombre: '', turno: 'mañana' as TurnoType, nivel: '', cupo: 20 }
...
export default function SalasPage() {
  ...
  function openEdit(sala: Sala) {
    setEditingSala(sala)
    setForm({ nombre: sala.nombre, turno: sala.turno, nivel: sala.nivel, cupo: sala.cupo })
    setError(null)
    setModalOpen(true)
  }
          ...
          {salas.map((sala) => (
            <div key={sala.id} className="card p-5 flex flex-col gap-4 group hover:shadow-md transition-shadow">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-bold text-gray-900 text-lg leading-tight">{sala.nombre}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                value={form.turno}
                onChange={(e) => setForm({ ...form, turno: e.target.value as TurnoType })}
                className="input"
              >
                {TURNOS.map((t) => (
                  <option key={t} value={t}>{turnoLabels[t]}</option>
                ))}
              </select>
            </div>
            <div>
