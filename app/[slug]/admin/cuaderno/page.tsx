...
import { Plus, Loader2, BookOpen, CheckCircle, AlertCircle, Info, UserRound } from 'lucide-react'
...
export default function AdminCuadernoPage() {
  ...
  const alumnosDeSala = useMemo(
    () => alumnos.filter((a) => a.salaActualId === salaId && a.activo !== false),
    [alumnos, salaId]
  )
  ...
  async function handleEnviarNotaIndividual(e: React.FormEvent) {
    e.preventDefault()
    if (!notaIndAlumnoId || !notaIndTitulo.trim() || !notaIndContenido.trim() || !salaId) return
    ...
    <div className="p-6 lg:p-8 animate-fade-in">
      ...
      <div className="flex items-start justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Comunicaciones por sala</p>
        </div>
        {salaId && (
      ...
      {/* Modal nueva nota para toda la sala */}
      <NuevaNotaModal
        open={modalOpen}
